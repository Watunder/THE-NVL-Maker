...
//参数处理
f.参数=new Dictionary();
f.参数.num=f.config_cgmode.locate.count;
f.参数.locate=[];
f.参数.locate.assign(f.config_cgmode.locate);
[endscript]
;--------------------------------------------------
;档案设定
;档案数
...
;--------------------------------------------------
*window
[window_middle width=800 height=600 title="CG版面设定"]
[line title="每页CG数" name="f.参数.num" x=30 y=50] [hlink target=*设定数量 hint="不超过30个"][emb exp="getTransStr('设定')"][endlink]
;根据数量设定locate
[iscript]
...
[commit]
[iscript]
f.config_cgmode.locate.assign(f.参数.locate);
[endscript]
