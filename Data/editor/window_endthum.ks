...
//将各层位置代入f.config_endmode.locate
...
//参数处理
f.参数=new Dictionary();
f.参数.num=f.config_endmode.locate.count;
f.参数.locate=[];
f.参数.locate.assign(f.config_endmode.locate);
[endscript]
;--------------------------------------------------
;档案设定
;档案数
...
;--------------------------------------------------
*window
[window_middle width=800 height=600 title="END版面设定"]
[line title="每页END数" name="f.参数.num" x=30 y=50] [hlink target=*设定数量 hint="不超过30个"][emb exp="getTransStr('设定')"][endlink]
;根据数量设定locate
[iscript]
...
[commit]
[iscript]
f.config_endmode.locate.assign(f.参数.locate);
[endscript]
